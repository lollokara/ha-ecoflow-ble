...
def parse_path(path_str, scale):
    ...
    while idx < len(cmds):
        ...
        if c == 'M':
            ...
            current_poly = [(cx, cy)]
            # If we were in a poly, store it?
            # SVG M starts a new subpath.
            ...
            # Subdivide bezier
            p0 = (cx, cy)
            steps = 20 # smooth enough for super-sampled
...
def render_icon(name, path):
    ...
    # Scale from 24 (MDI default) to SIZE_SS
    # Center it
    mdi_scale = SIZE_SS / 24.0
    # Add padding to avoid clip? No, mapped perfectly.

    polys = parse_path(path, mdi_scale)

    # Rasterize using winding number (odd-even)
    # Actually, simply checking point in polygon for every pixel is slow but robust
    # But for 192x192 image, it's 36k pixels. Fine for offline script.

    # Ray casting algorithm
    pixels = img.load()

    # Optimization: Iterate scanlines
    ...
    # Downsample to target size (48x48) with AA (Resample.BOX or AVG)
    # We want 8bpp output.
    img_sm = img.resize((ICON_SIZE, ICON_SIZE), resample=Image.Resampling.BOX)

    return img_sm

def generate_c_file():
    c_content = '#include "font_mdi.h"\n\n'
    h_content = '#ifndef FONT_MDI_H\n#define FONT_MDI_H\n\n#include <stdint.h>\n\n'
    ...
    for name, path in ICONS.items():
        ...
        # Output C array (8bpp)
        var_name = f"icon_{name.lower()}_48x48"
        data = list(img.getdata())

        c_content += f"const uint8_t {var_name}[{ICON_SIZE * ICON_SIZE}] = {{\n"
        ...
        h_content += f"extern const uint8_t {var_name}[{ICON_SIZE * ICON_SIZE}];\n"

    h_content += "\n#endif\n"

    with open("EcoflowSTM32F4/lib/bsp/Fonts/font_mdi.c", "w") as f:
        f.write(c_content)

    with open("EcoflowSTM32F4/lib/bsp/Fonts/font_mdi.h", "w") as f:
        f.write(h_content)
