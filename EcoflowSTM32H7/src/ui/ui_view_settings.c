...
void UI_DrawSlider(UI_Widget* w) {
    ...
    BSP_LCD_FillRect(0, w->x, w->y + w->h/2 - 4, w->w, 8, 0xFFFFFFFF); // TODO color // Track height 8px

    // Active Track
    int handle_x = w->x + (w->w * w->value) / 100;
    // BSP_LCD_SetTextColor(UI_COLOR_ACCENT_TEAL);
    BSP_LCD_FillRect(0, w->x, w->y + w->h/2 - 4, handle_x - w->x, 8, 0xFFFFFFFF); // TODO color
...
void UI_Render_Settings(void) {
    ...
    // Header
    BSP_LCD_DrawHLine(0, 0, 50, 800, 0xFFFFFFFF); // TODO color
