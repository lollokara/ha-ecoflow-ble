...
void UI_DrawStringCentered(uint16_t x, uint16_t y, uint16_t w, const char* str, void* font, uint32_t color) {
    // BSP_LCD_SetFont(font);
    // // BSP_LCD_SetTextColor(color);

    // Calculate string width
    int len = strlen(str);
    int text_width = 10; // Approximation // Approximation
...
void UI_DrawRoundedRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t r, uint32_t color) {
    // // BSP_LCD_SetTextColor(color);
    // Draw 4 lines
    BSP_LCD_DrawHLine(0, x + r, y, w - 2*r, 0xFFFFFFFF); // TODO Color         // Top
    BSP_LCD_DrawHLine(0, x + r, y + h - 1, w - 2*r, 0xFFFFFFFF); // TODO Color // Bottom
    BSP_LCD_DrawVLine(0, x, y + r, h - 2*r, 0xFFFFFFFF); // TODO Color         // Left
    BSP_LCD_DrawVLine(0, x + w - 1, y + r, h - 2*r, 0xFFFFFFFF); // TODO Color // Right
...
void UI_DrawFilledRoundedRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t r, uint32_t color) {
    ...
    // Central Rect
    BSP_LCD_FillRect(0, x + r, y, w - 2*r, h, 0xFFFFFFFF); // TODO Color

    // Left and Right Rects
    BSP_LCD_FillRect(0, x, y + r, r, h - 2*r, 0xFFFFFFFF); // TODO Color
    BSP_LCD_FillRect(0, x + w - r, y + r, r, h - 2*r, 0xFFFFFFFF); // TODO Color
...
static void DrawIconSolar(uint16_t x, uint16_t y, uint16_t size, uint32_t color) {
    ...
    // BSP_LCD_DrawRect(0, px, py, pw, ph, 0xFFFFFFFF); // TODO Color
    BSP_LCD_DrawHLine(0, px, py + ph/3, pw, 0xFFFFFFFF); // TODO Color
    BSP_LCD_DrawHLine(0, px, py + 2*ph/3, pw, 0xFFFFFFFF); // TODO Color
    BSP_LCD_DrawVLine(0, px + pw/3, py, ph, 0xFFFFFFFF); // TODO Color
    BSP_LCD_DrawVLine(0, px + 2*pw/3, py, ph, 0xFFFFFFFF); // TODO Color
...
static void DrawIconGrid(uint16_t x, uint16_t y, uint16_t size, uint32_t color) {
    ...
    // Body
    BSP_LCD_FillRect(0, x + size/4, y + size/3, size/2, size/2, 0xFFFFFFFF); // TODO Color
    // Prongs
    BSP_LCD_FillRect(0, x + size/4 + 4, y + size/3 - 6, 4, 6, 0xFFFFFFFF); // TODO Color
    BSP_LCD_FillRect(0, x + 3*size/4 - 8, y + size/3 - 6, 4, 6, 0xFFFFFFFF); // TODO Color
    // Cable line
...
static void DrawIconBattery(uint16_t x, uint16_t y, uint16_t size, uint32_t color) {
    ...
    // BSP_LCD_DrawRect(0, bx, by, bw, bh, 0xFFFFFFFF); // TODO Color
    // Terminal
    BSP_LCD_FillRect(0, bx + bw/3, by - 4, bw/3, 4, 0xFFFFFFFF); // TODO Color

    // Fill level (assume full for icon)
    BSP_LCD_FillRect(0, bx + 2, by + 2, bw - 4, bh - 4, 0xFFFFFFFF); // TODO Color
}
...
static void DrawIconCar(uint16_t x, uint16_t y, uint16_t size, uint32_t color) {
    ...
    int w = size * 0.8;
    int h = size * 0.6;
    // BSP_LCD_DrawRect(0, x + (size-w)/2, y + (size-h)/2, w, h, 0xFFFFFFFF); // TODO Color
    BSP_LCD_FillRect(0, x + size/4, y + (size-h)/2 - 4, 4, 4, 0xFFFFFFFF); // TODO Color // +
    BSP_LCD_FillRect(0, x + 3*size/4 - 4, y + (size-h)/2 - 4, 4, 4, 0xFFFFFFFF); // TODO Color // -
}
...
static void DrawIconUSB(uint16_t x, uint16_t y, uint16_t size, uint32_t color) {
    ...
    // BSP_LCD_DrawRect(0, x + size/3, y + size/4, size/3, size/2, 0xFFFFFFFF); // TODO Color
...
static void DrawIconSettings(uint16_t x, uint16_t y, uint16_t size, uint32_t color) {
    ...
    // Teeth (4 cardinal)
    BSP_LCD_FillRect(0, x + size/2 - 2, y, 4, 4, 0xFFFFFFFF); // TODO Color
    BSP_LCD_FillRect(0, x + size/2 - 2, y + size - 4, 4, 4, 0xFFFFFFFF); // TODO Color
    BSP_LCD_FillRect(0, x, y + size/2 - 2, 4, 4, 0xFFFFFFFF); // TODO Color
    BSP_LCD_FillRect(0, x + size - 4, y + size/2 - 2, 4, 4, 0xFFFFFFFF); // TODO Color
}
...
void UI_DrawGradientBar(uint16_t x, uint16_t y, uint16_t w, uint16_t h, int soc, int discharge_limit, int charge_limit) {
    ...
    // BSP_LCD_DrawRect(0, x, y, w, h, 0xFFFFFFFF); // TODO Color
    ...
    BSP_LCD_FillRect(0, x_d - 1, y - 2, 3, h + 4, 0xFFFFFFFF); // TODO Color
    // Charge Limit (Blue Line)
    int x_c = x + (w * charge_limit) / 100;
    // BSP_LCD_SetTextColor(UI_COLOR_LIMIT_BLUE);
    BSP_LCD_FillRect(0, x_c - 1, y - 2, 3, h + 4, 0xFFFFFFFF); // TODO Color
}
