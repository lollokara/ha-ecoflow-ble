...
bool Flash_WriteChunk(uint32_t offset, uint8_t *data, uint32_t length) {
    ...
    for (uint32_t i = 0; i < length; i += 4) {
        ...
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, start_addr + i, data_word) != HAL_OK) {
            return false;
        }
    }
    return true;
...
bool Flash_PrepareOTA(void) {
    ...
    if (active_bank == 0) {
        ...
        first_sector = FLASH_SECTOR_3;
        last_sector = FLASH_SECTOR_7;
    } else {
...
bool Flash_CopyBootloader(void) {
    ...
    if (active_bank == 0) {
        ...
        boot_sector = FLASH_SECTOR_5;
        config_sector = FLASH_SECTOR_6;
        dst_base = 0x08100000;
    } else {
    ...
    for (uint32_t i = 0; i < BOOTLOADER_SIZE; i += 4) {
        ...
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, dst + i, data) != HAL_OK) {
            return false;
        }
    }
