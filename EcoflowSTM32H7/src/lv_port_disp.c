...
#define LV_BUF_ADDR_1 (0x70000000 + 0x400000) // +4MB offset
#define LV_BUF_ADDR_2 (0x70000000 + 0x600000)
...
static void disp_init(void)
{
    ...
    // BSP_LCD_LayerDefaultInit(0, 0x70000000);
    ...
    hlcd_dma2d.Init.ColorMode = DMA2D_ARGB8888;
    hlcd_dma2d.Init.OutputOffset = 0; // Will be updated in flush
    HAL_DMA2D_Init(&hlcd_dma2d);
    HAL_DMA2D_ConfigLayer(&hlcd_dma2d, 1);
    hlcd_dma2d.LayerCfg[1].InputOffset = 0;
    hlcd_dma2d.LayerCfg[1].InputColorMode = DMA2D_ARGB8888;
}

static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    // Use DMA2D to copy from buffer to active LCD Framebuffer
    uint32_t dest_addr = hlcd_ltdc.LayerCfg[0].FBStartAdress;
    ...
    // hlcd_dma2d.Instance->OOR = offLine;
    ...
    {
         if (HAL_DMA2D_Start(&hlcd_dma2d, (uint32_t)color_p, dest_address, width, height) == HAL_OK)
         {
             HAL_DMA2D_PollForTransfer(&hlcd_dma2d, 10);
         }
    }
