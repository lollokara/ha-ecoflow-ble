...
void parsePacket(const Packet& pkt, EcoflowData& data) {
    ...
    // V2 Binary Packet (Delta 2)
        ...
        if (pkt.getSrc() == 0x02 && pkt.getCmdId() == 0x02) { // PD Heartbeat
            ...
            uint8_t num_batteries = p[0];
            int offset = 0;

            d2.extraBatteries.clear();
            for(int i=0; i < num_batteries; i++) {
                if (offset + sizeof(Delta2BmsPacket) > payload.size()) break;
                ...
                if (i == 0) { // Main battery
                    ...
                    extra.batteryLevel = bms->f32_show_soc;
                    extra.inputPower = bms->input_watts;
                    extra.outputPower = bms->output_watts;
                    extra.batteryTemperature = bms->temp;
                    extra.cycles = bms->cycles;
