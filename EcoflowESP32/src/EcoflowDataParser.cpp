...
// Auto-generated logger functions
static void logFullDelta3Data(const pd335_sys_DisplayPropertyUpload& msg) {
    ...
    if (msg.has_energy_backup_conflict_flag) LOG_STM_I(TAG, "energy_backup_conflict_flag: %d", (int)msg.energy_backup_conflict_flag);
    vTaskDelay(10);
    if (msg.has_self_powered_conflict_flag) LOG_STM_I(TAG, "self_powered_conflict_flag: %d", (int)msg.self_powered_conflict_flag);
    if (msg.has_tou_mode_conflict_flag) LOG_STM_I(TAG, "tou_mode_conflict_flag: %d", (int)msg.tou_mode_conflict_flag);
    if (msg.has_storm_pattern_conflict_flag) LOG_STM_I(TAG, "storm_pattern_conflict_flag: %d", (int)msg.storm_pattern_conflict_flag);
    if (msg.has_bypass_out_disable_conflict_flag) LOG_STM_I(TAG, "bypass_out_disable_conflict_flag: %d", (int)msg.bypass_out_disable_conflict_flag);
    ...
    if (msg.has_backup_reverse_soc) LOG_STM_I(TAG, "backup_reverse_soc: %d", (int)msg.backup_reverse_soc);
