...
void updateBrightness() {
    ...
    // Map to 4% - 50% (10.0 - 127.5) - Bump min further to ensure night mode visibility
    // strip.setBrightness takes 0-255
    float target = 10.0f + (norm * (177.5f - 10.0f));
...
void drawDashboard(DeviceSlot* slotD3, DeviceSlot* slotW2, DeviceSlot* slotD3P, DeviceSlot* slotAC) {
    ...
    if (isNightMode) {
        ...
        // Blink: 1s ON, 1s OFF
        if ((millis() / 1000) % 2 == 0) {
            setPixel(19, 8, cWhite); // Use White for better visibility at low brightness
        }
    }
}
...
void drawDetailMenu(DeviceType activeType) {
    ...
    switch (activeType) {
        ...
        case DeviceType::DELTA_PRO_3:
            acOn = currentData.deltaPro3.acHvPort;
            acOut = (int)(currentData.deltaPro3.acHvOutputPower);
