...
Packet::Packet(uint8_t src, uint8_t dest, uint8_t cmdSet, uint8_t cmdId,
               const std::vector<uint8_t> &payload, uint8_t check_type,
               uint8_t encrypted, uint8_t version, uint32_t seq,
               uint8_t product_id)
    : _src(src), _dest(dest), _cmdSet(cmdSet), _cmdId(cmdId), _payload(payload),
      _check_type(check_type), _encrypted(encrypted), _version(version),
      _seq(seq), _product_id(product_id) {}
...
Packet *Packet::fromBytes(const uint8_t *data, size_t len, bool is_xor) {
  ...
  return new Packet(src, dest, cmd_set, cmd_id, payload, dsrc, ddest, version,
                    seq, product_id);
}
...
EncPacket::EncPacket(uint8_t frame_type, uint8_t payload_type,
                     const std::vector<uint8_t> &payload, uint8_t needs_ack,
                     uint8_t is_ack)
...
std::vector<Packet> EncPacket::parsePackets(const uint8_t *data, size_t len,
  ...
  while (rxBuffer.size() >= 8) { // Minimum size for an EncPacket header + CRC
    if (rxBuffer[0] != (PREFIX & 0xFF) ||
        rxBuffer[1] != ((PREFIX >> 8) & 0xFF)) {
    ...
    uint16_t crc_from_packet =
        rxBuffer[total_len - 2] | (rxBuffer[total_len - 1] << 8);
    ...
    std::vector<uint8_t> raw_payload(rxBuffer.begin() + 6,
                                     rxBuffer.begin() + 6 + payload_len);
    std::vector<uint8_t> processed_payload;

    if (isAuthenticated) {
      processed_payload.resize(raw_payload.size());
      crypto.decrypt_session(raw_payload.data(), raw_payload.size(),
                             processed_payload.data());
      ...
      if (!processed_payload.empty()) {
        ...
        if (padding > 0 && padding <= 16 &&
            processed_payload.size() >= padding) {
    ...
    Packet *packet = Packet::fromBytes(
        processed_payload.data(), processed_payload.size(),
        isAuthenticated); // is_xor depends on auth state (Delta 3 uses it,
                          // others might not during handshake)
    if (packet) {
