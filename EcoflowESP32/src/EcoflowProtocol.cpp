...
Packet* Packet::fromBytes(const uint8_t* data, size_t len, bool is_xor) {
  ...
  if (header_crc_calc != data[HEADER_SIZE]) {
    return nullptr;
  }
  
  std::vector<uint8_t> payload;
  if (payload_len > 0) {
    if (len < HEADER_SIZE + 1 + payload_len + 2) {
      return nullptr;
    }
    ...
    if (payload_crc_calc != payload_crc_received) {
      return nullptr;
    }
  }
...
EncPacket* EncPacket::fromBytes(const uint8_t* data, size_t len,
  ...
  if (data[0] != PREFIX_0 || data[1] != PREFIX_1) {
    return nullptr;
  }
...
// ============================================================================
// SESSION KEY GENERATION (using mbedtls MD5)
// ============================================================================
...
void EcoflowECDH::generateSessionKey(const uint8_t* sRand, const uint8_t* seed,
  ...
  std::vector<uint8_t> shared_key_16(shared_key.begin(), shared_key.begin() + 16);
  ...
  session_input.insert(session_input.end(), shared_key_16.begin(), shared_key_16.end());
...
// ============================================================================
// COMMAND BUILDERS (using mbedtls MD5)
// ============================================================================

std::vector<uint8_t> EcoflowCommands::buildGetKeyInfoReq() {
  EncPacket pkt(ENCPACKET_FRAME_TYPE_COMMAND, ENCPACKET_PAYLOAD_TYPE_VX_PROTOCOL,
                (const uint8_t*)"\x02", 1);
  return pkt.toBytes();
}

std::vector<uint8_t> EcoflowCommands::buildGetAuthStatus() {
  Packet pkt(0x21, 0x35, PACKET_CMD_SET_DEFAULT, PACKET_CMD_ID_GET_AUTH_STATUS,
             nullptr, 0, 0x01, 0x01, 0x03);
  return pkt.toBytes();
}

std::vector<uint8_t> EcoflowCommands::buildAutoAuthentication(const std::string& user_id,
  ...
  mbedtls_md5_update(&md5, (const uint8_t*)combined.data(), combined.size());
  ...
  for (int i = 0; i < 16; i++) {
    ...
    payload.push_back(hex[0]);
    payload.push_back(hex[1]);
  }
  
  Packet pkt(0x21, 0x35, PACKET_CMD_SET_DEFAULT, PACKET_CMD_ID_AUTO_AUTH,
             payload.data(), payload.size(), 0x01, 0x01, 0x03);
  return pkt.toBytes();
}

std::vector<uint8_t> EcoflowCommands::buildStatusRequest() {
  Packet pkt(0x21, 0x35, PACKET_CMD_SET_DEFAULT, PACKET_CMD_ID_REQUEST_STATUS,
             nullptr, 0, 0x01, 0x01, 0x03);
  return pkt.toBytes();
}

std::vector<uint8_t> EcoflowCommands::buildAcCommand(bool on) {
  uint8_t payload[8] = {0x34, static_cast<uint8_t>(on ? 0x01 : 0x00), 0, 0, 0, 0, 0, 0};
  
  Packet pkt(0x21, 0x35, PACKET_CMD_SET_DEFAULT, PACKET_CMD_ID_AC_CONTROL,
             payload, sizeof(payload), 0x01, 0x01, 0x03);
  return pkt.toBytes();
}

std::vector<uint8_t> EcoflowCommands::buildDcCommand(bool on) {
  uint8_t payload[8] = {0x35, static_cast<uint8_t>(on ? 0x01 : 0x00), 0, 0, 0, 0, 0, 0};
