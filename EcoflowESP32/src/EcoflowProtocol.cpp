...
std::vector<Packet> EncPacket::parsePackets(const uint8_t* data, size_t len, EcoflowCrypto& crypto) {
    ...
    while (buffer.size() >= 8) {
        ...
        if (packet) {
            ESP_LOGD(TAG, "Successfully parsed packet: cmd_set=0x%02x, cmd_id=0x%02x", packet->getCmdSet(), packet->getCmdId());
            packets.push_back(*packet);
            delete packet;
        } else {
            ESP_LOGW(TAG, "Failed to parse packet from decrypted payload");
        }
