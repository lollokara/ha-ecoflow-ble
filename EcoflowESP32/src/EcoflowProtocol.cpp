...
EncPacket::EncPacket(uint8_t frame_type, uint8_t payload_type, const std::vector<uint8_t>& payload, uint16_t seq, uint8_t device_sn, const uint8_t* key, const uint8_t* iv) :
    ...
    if (key && iv) {
        ...
        mbedtls_aes_crypt_cbc(&aes, MBEDTLS_AES_ENCRYPT, padded_payload.size(), (uint8_t*)iv, padded_payload.data(), _payload.data());
...
EncPacket* EncPacket::fromBytes(const uint8_t* data, size_t len, const uint8_t* key, const uint8_t* iv) {
    ...
    if(key && iv) {
        ...
        mbedtls_aes_crypt_cbc(&aes, MBEDTLS_AES_DECRYPT, payload.size(), (uint8_t*)iv, payload.data(), decrypted_payload.data());
        mbedtls_aes_free(&aes);
...
namespace EcoflowCommands {
    ...
    std::vector<uint8_t> buildAuthStatusRequest() {
        ...
        EncPacket pkt(EncPacket::FRAME_TYPE_PROTOCOL, EncPacket::PAYLOAD_TYPE_VX_PROTOCOL, inner_pkt.toBytes(), 0, 0, nullptr, nullptr);
        return pkt.toBytes();
    }
