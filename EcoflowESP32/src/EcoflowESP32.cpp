 ...
 * It uses a dedicated FreeRTOS task to handle all BLE operations
 * non-blockingly.
 */
 ...
 */
void EcoflowESP32::notifyCallback(
  ...
  NimBLEClient *pClient =
      pRemoteCharacteristic->getRemoteService()->getClient();
...
EcoflowClientCallback::EcoflowClientCallback(EcoflowESP32 *instance)
    : _instance(instance) {}
...
bool EcoflowESP32::begin(const std::string &userId, const std::string &deviceSn,
...
void EcoflowESP32::ble_task_entry(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (self->_state != self->_lastState) {
      ESP_LOGI(TAG, "State changed: %d -> %d", (int)self->_lastState,
               (int)self->_state);
      ...
      if (millis() - self->_lastConnectionAttempt > 5000) { // 5s retry delay
        ...
        if (self->_connectionRetries < MAX_CONNECT_ATTEMPTS) {
          ESP_LOGI(TAG, "Connecting... (Attempt %d/%d)",
                   self->_connectionRetries + 1, MAX_CONNECT_ATTEMPTS);
    ...
    if (self->_pClient->isConnected()) {
      ...
      case ConnectionState::SERVICE_DISCOVERY: {
        NimBLERemoteService *pSvc =
            self->_pClient->getService("00000001-0000-1000-8000-00805f9b34fb");
        if (pSvc) {
          self->_pWriteChr =
              pSvc->getCharacteristic("00000002-0000-1000-8000-00805f9b34fb");
          self->_pReadChr =
              pSvc->getCharacteristic("00000003-0000-1000-8000-00805f9b34fb");
      ...
      case ConnectionState::SUBSCRIBING_NOTIFICATIONS:
        if (self->_pReadChr->canNotify() &&
            self->_pReadChr->subscribe(true, notifyCallback)) {
...
void EcoflowESP32::_startAuthentication() {
  ...
  _txSeq = 1; // Initialize to non-zero
  ...
  payload.insert(payload.end(), pub_key,
                 pub_key + _crypto.get_public_key_len());

  EncPacket enc_packet(EncPacket::FRAME_TYPE_COMMAND,
                       EncPacket::PAYLOAD_TYPE_VX_PROTOCOL, payload);
  _sendCommand(enc_packet.toBytes());
}

/**
 * @brief Main packet handler, routes packets to the correct handler based on
 * auth state.
 */
void EcoflowESP32::_handlePacket(Packet *pkt) {
  ...
  if (isAuthenticated()) {
    ...
    if (pkt->getDest() == 0x21) {
      Packet reply(pkt->getDest(), pkt->getSrc(), pkt->getCmdSet(),
                   pkt->getCmdId(), pkt->getPayload(), 0x01, 0x01,
                   pkt->getVersion(), pkt->getSeq(), 0x0d);
      EncPacket enc_reply(EncPacket::FRAME_TYPE_PROTOCOL,
                          EncPacket::PAYLOAD_TYPE_VX_PROTOCOL, reply.toBytes());
...
void EcoflowESP32::_handleAuthPacket(Packet *pkt) {
  ...
  if (_state == ConnectionState::REQUESTING_AUTH_STATUS) {
    if (pkt->getCmdSet() == 0x35 && pkt->getCmdId() == 0x89) {
      ...
      uint8_t md5_data[16];
      mbedtls_md5((const unsigned char *)(_userId + _deviceSn).c_str(),
                  _userId.length() + _deviceSn.length(), md5_data);
      char hex_data[33];
      for (int i = 0; i < 16; i++)
        sprintf(&hex_data[i * 2], "%02X", md5_data[i]);
      hex_data[32] = 0;
      std::vector<uint8_t> auth_payload(hex_data, hex_data + 32);
      Packet auth_pkt(0x21, 0x35, 0x35, 0x86, auth_payload, 0x01, 0x01,
                      _protocolVersion, _txSeq++, 0x0d);
      EncPacket enc_auth(EncPacket::FRAME_TYPE_PROTOCOL,
                         EncPacket::PAYLOAD_TYPE_VX_PROTOCOL,
                         auth_pkt.toBytes());
...
bool EcoflowESP32::_sendCommand(const std::vector<uint8_t> &command) {
  if (_pWriteChr && isConnected()) {
    ...
    _pWriteChr->writeValue(command.data(), command.size(),
...
void EcoflowESP32::_sendConfigPacket(const pd335_sys_ConfigWrite &config) {
  ...
  Packet packet(0x20, 0x02, 0xFE, 0x11, payload, 0x01, 0x01, _protocolVersion,
                _txSeq++);
  EncPacket enc_packet(EncPacket::FRAME_TYPE_PROTOCOL,
                       EncPacket::PAYLOAD_TYPE_VX_PROTOCOL, packet.toBytes());
...
int EcoflowESP32::getBatteryLevel() {
  if (_protocolVersion == 2)
    return _data.wave2.batSoc;
  return (int)_data.delta3.batteryLevel;
}
int EcoflowESP32::getInputPower() {
  if (_protocolVersion == 2)
    return (_data.wave2.batPwrWatt > 0) ? _data.wave2.batPwrWatt : 0;
  return (int)_data.delta3.inputPower;
}
int EcoflowESP32::getOutputPower() {
  if (_protocolVersion == 2)
    return (_data.wave2.batPwrWatt < 0) ? abs(_data.wave2.batPwrWatt) : 0;
...
bool EcoflowESP32::isConnecting() {
  return (_state >= ConnectionState::CREATED &&
          _state < ConnectionState::AUTHENTICATED);
}
...
bool EcoflowESP32::requestData() {
  if (!isAuthenticated())
    return false;
  Packet packet(0x01, 0x02, 0xFE, 0x11, {}, 0x01, 0x01, _protocolVersion,
                _txSeq++, 0x0d);
  EncPacket enc_packet(EncPacket::FRAME_TYPE_PROTOCOL,
                       EncPacket::PAYLOAD_TYPE_VX_PROTOCOL, packet.toBytes());
...
bool EcoflowESP32::setAcChargingLimit(int watts) {
  ...
  config.cfg_ac_in_chg_mode =
      pd335_sys_AC_IN_CHG_MODE_AC_IN_CHG_MODE_SELF_DEF_POW;
