...
bool EcoflowESP32::begin(const std::string& userId, const std::string& deviceSn, const std::string& ble_address, uint8_t protocolVersion) {
    ...
    if (_deviceSn.rfind("MR51", 0) == 0) _deviceType = DeviceType::DELTA_PRO_3;
    else if (_deviceSn.rfind("DC01", 0) == 0 || _deviceSn.rfind("F371", 0) == 0) _deviceType = DeviceType::ALTERNATOR_CHARGER;
    else if (_deviceSn.rfind("KT", 0) == 0) _deviceType = DeviceType::WAVE_2;
    else _deviceType = DeviceType::DELTA_3;
...
int EcoflowESP32::getBatteryLevel() {
    if (_deviceType == DeviceType::WAVE_2) return _data.wave2.batSoc;
...
int EcoflowESP32::getInputPower() {
    if (_deviceType == DeviceType::WAVE_2) return (_data.wave2.batPwrWatt > 0) ? _data.wave2.batPwrWatt : 0;
...
int EcoflowESP32::getOutputPower() {
    if (_deviceType == DeviceType::WAVE_2) return (_data.wave2.batPwrWatt < 0) ? abs(_data.wave2.batPwrWatt) : 0;
...
int EcoflowESP32::getSolarInputPower() {
    if (_deviceType == DeviceType::WAVE_2) return _data.wave2.mpptPwrWatt;
...
int EcoflowESP32::getDcOutputPower() {
    if (_deviceType == DeviceType::WAVE_2) return _data.wave2.psdrPwrWatt;
...
int EcoflowESP32::getCellTemperature() {
    if (_deviceType == DeviceType::WAVE_2) return (int)_data.wave2.outLetTemp;
...
int EcoflowESP32::getAmbientTemperature() {
    if (_deviceType == DeviceType::WAVE_2) return (int)_data.wave2.envTemp;
    return 0;
}
...
bool EcoflowESP32::isAcOn() {
    if (_deviceType == DeviceType::WAVE_2) return (_data.wave2.mode != 0);
    if (_deviceType == DeviceType::DELTA_PRO_3) return (_data.deltaPro3.acLvPort || _data.deltaPro3.acHvPort);
    return _data.delta3.acOn;
}
bool EcoflowESP32::isDcOn() {
    if (_deviceType == DeviceType::WAVE_2) return (_data.wave2.powerMode != 0);
    if (_deviceType == DeviceType::DELTA_PRO_3) return _data.deltaPro3.dc12vPort;
    return _data.delta3.dcOn;
}
bool EcoflowESP32::isUsbOn() {
    if (_deviceType == DeviceType::DELTA_PRO_3) return false; // Not in data yet
...
bool EcoflowESP32::requestData() {
    ...
    uint8_t dest = 0x02;
    if (_deviceType == DeviceType::WAVE_2) dest = 0x42;
    else if (_deviceType == DeviceType::ALTERNATOR_CHARGER) dest = 0x14;
