...
bool EcoflowESP32::_sendCommand(const ToDevice& message) {
    if (!isConnected() || !_authenticated) return false;
...
bool EcoflowESP32::_authenticate() {
    ...
    authFrame[2] = 0x01;
    authFrame[3] = 0x01;
    memcpy(authFrame + 4, secretKey, 16);
    ...
    if (!pWriteChr->writeValue(authFrame, sizeof(authFrame), true)) {
        Serial.println("Failed to send auth frame");
        return false;
    }

    // In the v2 protocol, we don't wait for a specific auth response.
    // If the device doesn't disconnect us, we assume success.
    _authenticated = true;
    Serial.println("Authentication frame sent.");
    return true;
}
...
void EcoflowESP32::parse(uint8_t* pData, size_t length) {
    ...
    if (pb_decode(&proto_stream, FromDevice_fields, &message)) {
        if (message.which_pdata == FromDevice_all_data_tag) {
