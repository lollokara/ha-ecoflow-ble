    ...
    while(1) {
        ...
        if (cmd == CMD_OTA_START) {
            ...
            for (uint32_t sec = start_sector; sec <= end_sector; sec++) {
                ...
                // Refresh IWDG if needed
                 __HAL_IWDG_RELOAD_COUNTER(&hiwdg); // Assuming hiwdg is not init here but maybe system watchdog
            ...
            // Add to checksum (simple sum for now, or match verify logic)
            // For now, we trust the write if HAL_OK.
            ...
            // Verify and Swap
            // Here we should ideally check the checksum of the whole bank.
            // But for now, we assume integrity if CRC passed on all chunks.
            ...
            // Toggle BFB2
            if (bank2_active) {
                // Was Bank 2, Switch to Bank 1
                 OBInit.USERConfig &= ~FLASH_OPTCR_BFB2;
            } else {
                // Was Bank 1, Switch to Bank 2
                 OBInit.USERConfig |= FLASH_OPTCR_BFB2;
            }
