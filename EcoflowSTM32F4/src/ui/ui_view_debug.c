...
static void add_list_item(lv_obj_t * parent, const char * name, const char * val) {
    ...
    lv_obj_clear_flag(item, LV_OBJ_FLAG_SCROLLABLE); // Fix: Prevent unwanted scrolling on items
...
static void populate_device_list(void) {
    ...
    // Save Scroll Position before clean
    lv_coord_t scroll_y = lv_obj_get_scroll_y(cont_list);

    lv_obj_clean(cont_list); // Clear previous items to avoid duplication
    ...
    // We must force layout update first
...
// Function to call periodically (every 5s) to refresh debug info
void UI_Debug_PeriodicRefresh(void) {
    if (scr_debug) {
         populate_device_list(); // Re-populate to update dynamic values
         UART_SendGetDebugInfo();
    }
}
