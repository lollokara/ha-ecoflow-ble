...
static void event_power_toggle(lv_event_t * e) {
    ...
    // Button state is now managed solely by updates, so current "checked" means ON.
    // We want to invert it.
    bool state = lv_obj_has_state(btn, LV_STATE_CHECKED);
    send_cmd(W2_PARAM_POWER, state ? 0 : 1);
}
...
void ui_view_wave2_init(lv_obj_t * parent) {
    ...
    // Removed LV_OBJ_FLAG_CHECKABLE to prevent local toggle
