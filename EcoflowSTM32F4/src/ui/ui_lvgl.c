...
void UI_LVGL_Update(DeviceStatus* dev) {
    ...
    if (is_main_device) {
        ...
        float startVoltage = get_float_aligned(&dev->data.ac.startVoltage);
        if (startVoltage > 0) {
            int val = (int)(startVoltage * 10); // Float to Decivolt
            if (val != alt_start_v) {
                alt_start_v = val;
        ...
        int powLim = get_int32_aligned(&dev->data.ac.powerLimit);
        if (powLim > 0) {
            int val = powLim;
            if (val != alt_pow_limit) {
                alt_pow_limit = val;
