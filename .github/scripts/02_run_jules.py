...
def main():
    ...
    for task in tasks:
        ...
        2. BEST PRACTICES EMBEDDED: 
           - Usa tipi a dimensione fissa (es. uint8_t, int32_t).
           - EVITA categoricamente l'allocazione dinamica (String, malloc) se non strettamente necessaria; prediligi buffer statici o array (char[]).
           - NON usare funzioni bloccanti come delay(). Usa millis() per logiche temporali non bloccanti.
