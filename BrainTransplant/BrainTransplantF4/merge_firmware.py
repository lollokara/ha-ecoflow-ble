...
def merge_firmware(source, target, env):
    # Paths - Adjusted for BrainTransplant directory structure
    # BrainTransplantF4 is sibling to BrainTransplantCore
    ...
    # Constants
    BOOTLOADER_SIZE = 0x4000 # 16KB (Sector 0)
    CONFIG_SIZE = 0x4000     # 16KB (Sector 1)
    APP_OFFSET = 0x8000      # 32KB (Sector 2+)

    print(f"Merging firmware... ")
    print(f"App Path: {app_path}")

    if not os.path.exists(bootloader_path):
        print(f"Warning: Bootloader not found at {bootloader_path}. Please build BrainTransplantCore first.")
        # Do not fail build, just skip merge if bootloader is missing
        return
