    ...
    while (Serial1.available()) {
        ...
        if (!collecting) {
            ...
            if (rx_idx == 3) { // START, CMD, LEN
                ...
                if (expected_len > 250) { collecting = false; rx_idx = 0; }
            } else if (rx_idx > 3) {
                if (rx_idx == (4 + expected_len)) {
                    uint8_t recv_crc = rx_buf[rx_idx - 1];
                    uint8_t calc_crc = calculate_crc8(&rx_buf[1], 2 + expected_len);
                    if (recv_crc == calc_crc) {
                        processPacket(rx_buf, rx_idx);
                    }
                    collecting = false; rx_idx = 0;
                }
            }
            if (rx_idx >= sizeof(rx_buf)) { collecting = false; rx_idx = 0; }
        }
    }
}
...
void Stm32Serial::processPacket(uint8_t* rx_buf, uint8_t len) {
    ...
    if (cmd == CMD_OTA_ACK) otaAckReceived = true;
    else if (cmd == CMD_OTA_NACK) otaNackReceived = true;
}

// --- Helpers to pack data ---
static int pack(uint8_t* buf, uint8_t cmd, const uint8_t* payload, size_t payload_len) {
    buf[0] = START_BYTE;
    buf[1] = cmd;
    buf[2] = (uint8_t)payload_len;
    if (payload && payload_len > 0) memcpy(&buf[3], payload, payload_len);
    buf[3 + payload_len] = calculate_crc8(&buf[1], 2 + payload_len);
...
void Stm32Serial::otaTask(void* parameter) {
    ...
    // Calc CRC32
    while(f.available()) {
        int readLen = f.read(buf, sizeof(buf));
    ...
    // 1. Send Start
    memcpy(buf, &totalSize, 4);
    int pLen = pack(packet, CMD_OTA_START, buf, 4);

    bool startSuccess = false;
    for(int i=0; i<3; i++) {
        self->sendData(packet, pLen);
        otaAckReceived = false; otaNackReceived = false;
        uint32_t t = millis();
        while(!otaAckReceived && !otaNackReceived && (millis() - t < 30000)) vTaskDelay(10);
    ...
    while(f.available()) {
        int readBytes = f.read(buf + 4, 200); // Leave room for offset
        memcpy(buf, &offset, 4); // Offset at beginning of payload
        pLen = pack(packet, CMD_OTA_CHUNK, buf, 4 + readBytes);
        ...
        for(int i=0; i<3; i++) {
            ...
            otaAckReceived = false; otaNackReceived = false;
            uint32_t t = millis();
            while(!otaAckReceived && !otaNackReceived && (millis() - t < 2000)) vTaskDelay(5);
            if(otaAckReceived) { chunkSuccess = true; break; }
            vTaskDelay(50);
        }

        if(!chunkSuccess) { fail = true; break; }
        offset += readBytes;

        ota_progress = (offset * 100) / totalSize;
        if(ota_progress != last_prog) {
            // Log if needed
            last_prog = ota_progress;
        }
    }
    f.close();
    ...
    if(!fail) {
        memcpy(buf, &crc, 4);
        pLen = pack(packet, CMD_OTA_END, buf, 4);
        bool endSuccess = false;
        for(int i=0; i<3; i++) {
            self->sendData(packet, pLen);
            otaAckReceived = false; otaNackReceived = false;
            uint32_t t = millis();
            while(!otaAckReceived && !otaNackReceived && (millis() - t < 5000)) vTaskDelay(50);
            if(otaAckReceived) { endSuccess = true; break; }
        }

        if(endSuccess) {
            pLen = pack(packet, CMD_OTA_APPLY, NULL, 0);
            self->sendData(packet, pLen);
            ota_state = 3; ota_msg = "Rebooting...";
        } else {
